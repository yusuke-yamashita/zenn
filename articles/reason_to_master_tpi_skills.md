...
私は「TPI」という、比較的昔に作られたテストプロセス改善技術を取り扱う専門家です。
...
しかし、これらの意見を聞いて、正直、「どうせプロとして試してもないし上辺くらいしか理解してないのによくそんなこと言えんな」と思ったことは否定できません。
...
テストプロセス改善技術の導入するにはそのテストプロセス改善技術に習熟した人が主導して行なうべきだと考えます。
そのため、テストプロセス改善技術がプロに依存したトップダウンアプローチであるということは否定できません。
...
こういった工程について、「突然コンサル的な人がやってきて、第三者的に現状をチェックして、改善策を押しつけてくる」といったような印象を持ってしまうこともあります。
しかし、実際には、それはテストプロセス改善コンサルが得意とする関わりによって異なります。
例えば、上記の工程の見方を変えると、OODAループにそっくりです。
...
あるいは、もう少し熟達したテストプロセス改善コンサルであれば、テストプロセスモデルの内容から、コンテキストにによって取捨選択するなどの柔軟な対応ができるべきです。

そして、コンサルとしてこれからテストプロセスを改善していく人にも考えて欲しいことがあります。
標準をただ盲目に従うのではなく、標準について本質的な理解を深め、そのプロセスを実施しないことによる問題やリスクをきちんと理解して、納得できるよう説明できているべきだということです。
...
ただ、「実践での失敗のみから学ぶ組織」というのも健全ではないと考えます。
...
これらは単純にJSTQBやTMap NEXTなどのプロセスモデルを学ぶだけでは得られない、「プロセスをインスタンス化する力」が得られます。
...
2025年現在、シフトレフトやホリスティックテストなどのアジャイルテストの技法のほとんどは、既存のテストの技術の応用でできていると私は考えています。
...
テストエンジニアを含むエンジニアとの対話を重視して、問いをベースにその人の「ありたい姿」を言語化し、納得感を醸成しながら進んでいきます。
