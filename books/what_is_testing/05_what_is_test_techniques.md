...
と、思っていたら、株式会社ベリサーブの長谷川さんが私の言いたいことを端的に表した記事を記載されていました。
...
<!--無視-->そしてもうひとつ付け加えるならば、カバレッジは「テストケース」によってカバーされると考えます。
...
また、あるカバレッジ基準を満たすテストケースとして、それぞれ重要度は同じです。
...
この記事では上記の定義や使われ方を尊重したうえで、「テスト技法のキモはカバレッジによる網羅である」というスタンスを取りたいです。
...
テスト技法としては「デシジョンテーブル作成」「状態移動表や図の作成」はスコープに含まれていないと思っています。
元々ソフトウェア設計技法として「デシジョンテーブル」「状態移動」というものがあり、これらをカバレッジするテストケースを生成するのがテスト技法であるという認識です。

**ただこれは「デシジョンテーブル作成」「状態移動表や図の作成」がテストエンジニアに不要と言っているわけではありません。**

ソフトウェア開発において、デシジョンテーブルや状態移動図が必ずしも作成されるとは限りません。
...
また、テスト技法として「デシジョンテーブル作成」「状態移動図の作成」は含まれていないという立場の人間ですが、エンジニアとして持っていて悪いことはないスキルだと思っています。
...
探索的テストは本来的にテスト対象から学び、テストケースの作成と評価を同時に行なうからです。
この場合のテストモデルは「テスターが認知したソフトウェアのふるまい」になるのでしょうか。
謎。

何にせよ「テストケースを導出できる」という点で、「探索的テスト」を使うのならばテスト技法の仲間に入れてあげてもいいと思っています。
...
テスト技法だけでなく、さまざまな技術を用いてテストを行ないます。
