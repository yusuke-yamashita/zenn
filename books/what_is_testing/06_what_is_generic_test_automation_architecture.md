...
この世代の特徴は、テスト生成からテスト適合まで1つのツールで完結して、gTAAすべてのレイヤーが含まれることです。
機能分割/データ駆動などの構造化技法が使われず、「テスターの動作をリプレイする」という性質を持ったテスト自動化技術の世代です。

## 機能分割/データ駆動の世代
ページオブジェクトモデルなどを使用した、構造化プログラミングが自動化でも行なわれた時代だと認識しました。
...
しかしながら、最近流行なっているSaaS自動テストプロダクトはそんな方向性にはいっていないように思えます。
